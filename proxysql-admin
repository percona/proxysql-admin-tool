...
usage () {
  echo "Usage: [ options ]"
  echo "Options:"
    echo " --user=user_name, -u user_name         User to use when connecting to the ProxySQL service"
    echo " --password[=password], -p[password]    Password to use when connecting to the ProxySQL service"
    echo " --port=port_num, -P port_num           Port to use when connecting to the ProxySQL service"
    echo " --host=host_name, -h host_name         Hostname to use when connecting to the ProxySQL service"
...
if ! getopt --test
  then
  go_out="$(getopt --options=u:p::h:P:ed \
  --longoptions=user:,password::,host:,port:,enable,disable,start,stop,status,help: \
  --name="$(basename "$0")" -- "$@")"
  test $? -eq 0 || exit 1
  eval set -- $go_out
fi
...
proxysql_connection_check(){
  if ! mysql -u$usr $pass $hostname $port -e"show tables" >/dev/null 2>/dev/null; then 
...
pxc_connection_check(){
  ...
  read -p "<username>:<password>:<hostname>:<port> : " read_param 
  conn_array=(${read_param//:/ })
  pxc_pass=`echo $read_param | cut -d":" -f2`
  if [ -z $pxc_pass ];then
    echo -e "\nAlert! You have given empty password!"
    DSN="-u${conn_array[0]} -h${conn_array[1]} -P${conn_array[2]} --protocol=tcp"
  else
    DSN=" -u${conn_array[0]} -p${conn_array[1]} -h${conn_array[2]} -P${conn_array[3]} --protocol=tcp"
  fi
...
enable_proxysql(){
  ...
  echo "update global_variables set variable_value='$mon_uname' where variable_name='mysql-monitor_username'; update global_variables set variable_value='$mon_password' where variable_name='mysql-monitor_password'; " | mysql -u$usr $pass $hostname $port 2>/dev/null
  check_cmd $?  "Cannot set the mysql-monitor variables in ProxySQL"
  echo "LOAD MYSQL VARIABLES TO RUNTIME;SAVE MYSQL VARIABLES TO DISK;" | mysql -u$usr $pass $hostname $port 2>/dev/null

  # Adding Percona XtraDB Cluster nodes to ProxySQL
  echo -e "\n\nAdding the Percona XtraDB Cluster server nodes to ProxySQL"
  echo "DELETE FROM mysql_servers WHERE hostgroup_id=10" | mysql -u$usr $pass $hostname $port 2>/dev/null
  ...
  for i in "${wsrep_address[@]}"; do	
    ...
    echo "INSERT INTO mysql_servers (hostname,hostgroup_id,port,weight) VALUES ('$ws_ip',10,$ws_port,1000);" | mysql -u$usr $pass $hostname $port 2>/dev/null
    check_cmd $? "Failed to add the Percona XtraDB Cluster server node $ws_ip:$ws_port"
  done
  echo "LOAD MYSQL SERVERS TO RUNTIME; SAVE MYSQL SERVERS TO DISK;" | mysql -u$usr $pass $hostname $port 2>/dev/null

  # Adding Percona XtraDB Cluster monitoring script

  echo "DELETE FROM SCHEDULER WHERE ID=10;" | mysql -u$usr $pass $hostname $port 2>/dev/null
  check_cmd $?
  echo "INSERT  INTO SCHEDULER (id,active,interval_ms,filename,arg1,arg2,arg3,arg4,arg5) VALUES (10,1,2000,'$PROXYSQL_GALERA_CHECK',10,10,${#wsrep_address[@]},1,'/var/lib/proxysql/galera-check.log');" | mysql -u$usr $pass $hostname $port 2>/dev/null
  check_cmd $? "Failed to add the Percona XtraDB Cluster monitoring scheduler in ProxySQL"
  echo "LOAD SCHEDULER TO RUNTIME;SAVE SCHEDULER TO DISK;" | mysql -u$usr $pass $hostname $port 2>/dev/null
  ...
  check_user=`mysql  $DSN -Bse"SELECT user,host FROM mysql.user where user='$pxc_uname' and host='%';"`

  if [[ -z "$check_user" ]]; then
    mysql  $DSN -e "GRANT CREATE, DROP, LOCK TABLES, REFERENCES, EVENT, ALTER, DELETE, INDEX, INSERT, SELECT, UPDATE, CREATE TEMPORARY TABLES, TRIGGER, CREATE VIEW, SHOW VIEW, ALTER ROUTINE, CREATE ROUTINE  ON test.* TO $pxc_uname@'%' IDENTIFIED BY '$pxc_password';" 2>/dev/null
    check_cmd $? "Cannot add Percona XtraDB Cluster user : $pxc_uname (GRANT)"
    echo "INSERT INTO mysql_users (username,password,active,default_hostgroup) values ('$pxc_uname','$pxc_password',1,10);LOAD MYSQL USERS TO RUNTIME;SAVE MYSQL USERS FROM RUNTIME;SAVE MYSQL USERS TO DISK;" | mysql -u$usr $pass $hostname $port 2>/dev/null
...
disable_proxysql(){
  proxysql_connection_check
  echo "DELETE FROM mysql_users WHERE default_hostgroup=10;" | mysql -u$usr $pass $hostname $port 2>/dev/null
...
start_daemon(){
  ...
  do
    check_proxysql
    current_hosts=(`mysql -u$usr $pass $hostname $port -Bse"SELECT hostname,port FROM mysql_servers WHERE status='ONLINE'" | sed 's|\t|:|g' | tr '\n' ' '`)
    wsrep_address=(`mysql  $DSN -Bse "SHOW STATUS LIKE 'wsrep_incoming_addresses'" 2>/dev/null | awk '{print $2}' | sed 's|,| |g'`)
    for i in "${wsrep_address[@]}"; do
...
if [ "$enable" == 1 -o "$disable" == 1 -o "$start_daemon"  == 1 -o "$stop_daemon" == 1 -o "$status_daemon" == 1 ]; then
  ...
  if [ "$start_daemon" == 1 ];then
    ...
    if [ -f $PIDFILE ]; then
      ...
      check_proxysql
      PXC_NODES_CHECK=`mysql -u$usr $pass $hostname $port -Bse"SELECT COUNT(*) FROM mysql_servers WHERE hostgroup_id=10;"`
