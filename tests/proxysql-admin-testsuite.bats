...
@test "run the check for --adduser" {
  ...
  run_check_user_command=$(mysql --user=$PROXYSQL_USERNAME -p$PROXYSQL_PASSWORD  --host=$PROXYSQL_HOSTNAME --port=$PROXYSQL_PORT -Bse "select 1 from mysql_users where username='proxysql_test_user1'" | awk '{print $0}')
...
@test "run the check for --syncusers" {
  ...
  proxysql_user_count=$(mysql --user=$PROXYSQL_USERNAME -p$PROXYSQL_PASSWORD  --host=$PROXYSQL_HOSTNAME --port=$PROXYSQL_PORT -Bse "select count(*) from mysql_users" | awk '{print $0}')
