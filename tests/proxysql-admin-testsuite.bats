...
@test "run the check for cluster size" {
  ...
  wsrep_cluster_count=$(mysql --user=$CLUSTER_APP_USERNAME -ppassw0rd  --host=localhost --port=6033 --protocol=tcp -Bse"show status like 'wsrep_cluster_size'" | awk '{print $2}')
